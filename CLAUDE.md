...
Three guards worth keeping: a keytool step checks the store password, alias and key password within seconds and names the wrong secret (it also prints byte vs char counts, never values, so a non-ASCII password is visible in the log; the key check uses `keytool -certreq -keypass`, because `-importkeystore` silently tries the store password first and so gave a false pass on 2026-09-13 with a wrong `KEY_PASSWORD`); the job runs `apksigner verify` before publishing (a broken secret would otherwise silently ship an uninstallable APK); and the keystore is shredded in an `if: always()` step so a failure part-way through leaves no key material on the runner.
